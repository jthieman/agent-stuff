...
import { FilteredFs } from "../src/wrappers/filtered-fs.ts";
...
describe("FilteredFs", () => {
  ...
  describe("custom filter", () => {
    it("filter function hides matching paths", async () => {
      ...
      // No paths contain "credentials" in this fixture, so all visible
    ...
    it("filter + exclude both apply", async () => {
      ...
      expect(await fs.exists("/.env")).toBe(false); // filter
      expect(await fs.exists("/config/key.pem")).toBe(false); // exclude
  ...
  describe("rm", () => {
    it("rm of excluded path with force is no-op", async () => {
      ...
      // inner still has it (we never reached the inner rm)
